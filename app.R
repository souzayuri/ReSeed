...
#plant_traits <- read.csv("Plant Traits database.csv", sep = ",") %>% 
#  dplyr::select(!c(1,4,5,6,14,15,45,49,50,51,53:65)) %>% 
...
plant_traits_table <- read_rds("plant_traits_table.rds")
names(plant_traits_table)


# plant traits simulated table --------------------------------------------


#dttable <- read.csv("Plant Traits database.csv", sep = ",") %>% 
#  mutate(Genus_1 = Genus,
#         Plant.trait.value = rnorm(1366, mean = 100, sd = 10),
#         Minimun.value = rnorm(1366, mean = 20, sd = 3),
#         Maximum.value = rnorm(1366, mean = 30, sd = 4)) %>% 
#  unite('Species', Genus:Species) %>% 
#  rename(Genus = Genus_1) %>% 
#  select(c(1,65,everything())) %>% 
#  select(1:3,7:12,14)
#names(dttable)
...
dispersores <- read_csv("dispersers_marina.csv") %>% 
  select(!1)

dispersores_df <- dispersores %>% 
  dplyr::mutate(mammals = as.numeric(stringr::str_replace(mammals, "mammals", "1")),
                birds = as.numeric(stringr::str_replace(birds, "birds", "1")),
                bats = as.numeric(stringr::str_replace(bats, "bats", "1")),
                insects = as.numeric(stringr::str_replace(insects, "insects", "1"))) %>% 
  tidyr::pivot_longer(cols = 2:5, names_to = "dispersers", values_to = "value") %>% 
  tidyr::drop_na(value) %>% 
  dplyr::group_by(Species, dispersers) %>% 
  dplyr::summarise(sum = sum(value)) %>% 
  dplyr::ungroup() %>% 
  dplyr::rename(Plant_specie = Species) %>% 
  dplyr::mutate(dispersers = stringr::str_replace_all(dispersers, c("birds" = "Birds", 
                                                                    "mammals" = "Mammals",
                                                                    "bats" = "Bats",
                                                                    "insects" = "Insects"))) 
...
#Syndrome <- plant_traits_table %>% 
#  unite('Species', Genus:Species) %>% 
#  select(c("Species","Dispersal.syndrome", "Dispersal.group")) %>% 
#  rename(Syndrome = Dispersal.syndrome) %>% 
#  add_row(Syndrome = "Zoochory", 
#          Dispersal.group = "Bats, Mammals, Insects" , 
#          Species = "Syagrus_romanzoffiana") %>% 
#  filter(!c(Species == "Syagrus_romanzoffiana" & Syndrome == "anemochory")) %>% 
#  mutate_all(na_if,"") %>% 
#  drop_na(Syndrome) %>% 
#  group_by(Species, Syndrome) %>% 
#  unique() %>% 
#  ungroup() %>% 
#  separate(Dispersal.group, c("a", "b", "c"))%>% 
#  select(1:5) %>% 
#  pivot_longer(cols = 3:5, values_to = "Dispersers") %>% 
#  select(c(2,4,1)) %>% 
#  mutate(Dispersers = stringr::str_replace(Dispersers, "aves", "birds"),
#         Species = stringr::str_replace(Species, "_", " "),
#         Dispersers = str_to_title(Dispersers),
#         Syndrome = str_to_title(Syndrome)) %>% 
#  group_by(Syndrome, Dispersers, Species) %>% 
#  unique() %>% 
#  ungroup() %>% 
#  mutate(number = 1:113) %>% 
#  filter(!row_number() %in% c(48,20,72,74,78,27,55,57,65,109,30,61,80,59,1,50,53,10))
             ...
             sidebarLayout(
               sidebarPanel(
                 selectInput(
                   ...
                   label = "Species:",
                   selected = "Chrysophyllum gonocarpum",
    ...
    tabPanel(title = "Datatable",
             ...
             sidebarLayout(
               checkboxInput(inputId = "show_data",
                             ...
                             value = TRUE),
               column(12, DTOutput('traitstable')))
             ),
...
# statisticals ------------------------------------------------------------
    ...
    tabPanel(title = "Statisticals",
           ...
           mainPanel(
             tabsetPanel(type = "tabs",
                         tabPanel(title = "Datatable",
                                  shinydashboard::box(DTOutput("datasttc"), width = 12
                                  )),
      ...
      tabPanel(title = "Corumbatai plots",
               ...
               mainPanel(
                  tabsetPanel(type = "tabs",
                              ...
                              tabPanel(title = "Datatable",
                                       shinydashboard::box(DTOutput("corumbtable"), width = 14)),
...
# datatable -------------------------------------------------------------------
  ...
  output$traitstable <- DT::renderDataTable(
    if(input$show_data) {
      datatable(
        plant_traits_table,
        filter = "top",
        ...
        options = list(
          initComplete = JS(
            ...
            "}"),
          order = list(61, 'desc'), #reorder by modifying data
          keys = TRUE,
          autoWidth = TRUE,
          columnDefs = list(
            #list(targets = -1, orderable = TRUE),
            list(className = 'dt-center', targets = "_all"),
            list(targets = c(1:65), width = '200px')),
          scrollX = TRUE,
          dom = 'Blfrtip',
          deferRender = TRUE,
          scrollY = 450,
          ...
          buttons = list(
            ...
            list(
              ...
              bom = TRUE,
              text = "Download",
              title = paste("plant_traits-", Sys.Date())
            )
          )
        )
      )
...
# Input: get which columns to use as wraps ----
  output$choose_wraps <- renderUI({
    colheads <- reactive(names(data()))
    selectizeInput("wrap_choice", "Choose a column to wrap by:", choices=c("no wrap", colheads()))
...
# output plots ------------
  ...
  output$x_axis_select <- renderUI({
    varSelectInput("x_axis_selection", "Choose X axis", selected = "Sample.size.value", data())
  })
  output$y_axis_select <- renderUI({
    varSelectInput("y_axis_selection", "Choose Y axis", selected = "Plant.trait.value", data())
  })
  #filter selection
...
# datatable ---------------------------------------------------------------
  ...
  output$datasttc <-  DT::renderDataTable(planttraits.sttc,
                                          selection = "none",
                                          class = 'cell-border stripe',
...
# Dispersers diagram plot ------------------------------------------------------
  ...
  output$syndrome <- collapsibleTree::renderCollapsibleTree({
    mycolors <- colorRampPalette(brewer.pal(8, "Set2"))(106)
    mycolors
...
# corumbatai table --------------------------------------------------------
  ...
  output$corumbtable <-  DT::renderDataTable(spp_plots_coor_traits,
                                             filter = "top",
                                             selection = "none",
                                             class = 'cell-border stripe',
                                             extensions = c("KeyTable", 'Scroller'),
